...
def getParameterUniqCheck(con,pulsar_id,parameterType_id,citation_id,value,timeDerivative,companionNumber):
    cur = con.cursor()
    parameterIDtemp = cur.execute("SELECT DISTINCT parameter_id FROM parameter WHERE pulsar_id LIKE ? AND parameterType_id LIKE ? AND citation_id LIKE ? AND value LIKE ? AND timeDerivative LIKE ? AND companionNumber LIKE ?",(pulsar_id,parameterType_id,citation_id,value,timeDerivative,companionNumber))
...
def addParameter(con,pulsar_id,citation_id,linkedSet_id,fitParameters_id,observingSystem_id,parameterType_id,timeDerivative,companionNumber,value,uncertainty,referenceTime,commit=True):
    cur = con.cursor()
    parameter_id = getParameterUniqCheck(con,pulsar_id,parameterType_id,citation_id,value,timeDerivative,companionNumber)
    if not parameter_id:
        try:
...
def addBinaryType(con,pulsar_id,citation_id,confidence,binaryTypeOptions_id=None,label=None,commit=True):
    ...
    binaryType_id = getBinaryType(con,pulsar_id,associationType_id,citation_id)
    if not binaryType_id:
        try:
            cur.execute("INSERT INTO binaryType (binaryType_id,pulsar_id,binaryTypeOptions_id,confidence,citation_id) VALUES (NULL,?,?,?,?)",(pulsar_id,binaryTypeOptions_id,citation_id,confidence))
            if(commit==True):
                con.commit()
...
def addSourceType(con,pulsar_id,citation_id,confidence,sourceTypeOptions_id=None,label=None,commit=True):
    ...
    if not sourceType_id:
        try:
            cur.execute("INSERT INTO sourceType (sourceType_id,pulsar_id,sourceTypeOptions_id,confidence,citation_id) VALUES (NULL,?,?,?,?)",(pulsar_id,sourceTypeOptions_id,citation_id,confidence))
            if(commit==True):
                con.commit()
...
def parseGeneralParameters(label,value,uncertainty,observingSystemID=None,widthFraction=None):
    ...
    labeltemp = parList[parList['oldlabel'].str.match(label)]['newlabel'].to_numpy()
    #print(label)
    if(len(labeltemp)==0):
        havelabel = 0
    else:
        havelabel = 1
        timeDerivative = int(parList[parList['oldlabel'].str.match(label)]['timeDerivative'].to_numpy()[0])
        companionNumber = int(parList[parList['oldlabel'].str.match(label)]['companionNumber'].to_numpy()[0])
        newlabel = parList[parList['oldlabel'].str.match(label)]['newlabel'].to_numpy()[0]
        label=newlabel
