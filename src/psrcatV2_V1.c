...
int loadMeasurements(int pulsarID,measurementStruct *measurement,sqlite3 *db);
...
int main(int argc,char *argv[])
{
  ...
  if (argc == 3)
    {
      ...
      strcpy(dbVer,argv[2]);
      //    printf("Setting Version and DbVer = %s\n",dbVer);
    }
  // Allocate memory
  ...
  distanceID = (int *)malloc(sizeof(int)*64);
  ...
  //  p=2998;
  {
	...
	printf("%-8s %-29s %s \n","PSRJ",jname,ref);
      ...
      nMeasurement = loadMeasurements(pulsarID[p],measurement,db);
      ...
      for (i=0;i<nMeasurement;i++)
	{
      ...
      // Distances
      sprintf(cmd,"SELECT distance_id FROM distance  WHERE distance.pulsar_id=%d;",pulsarID[p]);
      nDistance = runSQL_returnIntArray(cmd,db,distanceID);
      if (nDistance > 0)
	{
	  ...
	  for (i=0;i<nDistance;i++)
	    {
	      sprintf(cmd,"SELECT label FROM distance WHERE distance_id=%d",distanceID[i]);
	      runSQL_returnStr(cmd,db,distanceLabel);
	      sprintf(cmd,"SELECT value FROM distance WHERE distance_id=%d",distanceID[i]);
	      runSQL_returnStr(cmd,db,distanceValue);

	      sprintf(cmd,"SELECT ifnull(uncertainty,'NULL') FROM distance WHERE distance_id=%d",distanceID[i]);

	      runSQL_returnStr(cmd,db,errorValue);

	      sprintf(cmd,"SELECT v1label FROM citation LEFT JOIN distance ON distance.citation_id=citation.citation_id WHERE distance_id=%d",distanceID[i]);
      ...
      if (nType>0)
	{
	  int isRadio=0;
	  int countIt=0;
	  if (nType==1) // Don't print anything if only radio
	    {
	      sprintf(cmd,"SELECT label FROM sourceTypeOptions LEFT JOIN sourceType ON sourceType.sourceTypeOptions_id=sourceTypeOptions.sourceTypeOptions_id WHERE sourceType_id=%d",typeID[0]);
	  ...
	  for (i=0;i<nAssoc;i++)
	    {
	      ...
	      sprintf(cmd,"SELECT name FROM association WHERE association_id=%d",assocID[i]);     
